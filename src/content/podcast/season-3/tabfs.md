...
[1:06] **Omar:** But it is very much this idea of your browser is this weird mini operating system. And then your real operating system has all these tools. How do we kind of bring the browser out? And tab, I guess, is just the most concrete thing in your browser, and so that got promoted to be the title.

[1:23] **Omar:** Maybe part of it is also the browser extension APIs. If you go look at the Chrome extension API, the tab APIs are pretty prominently placed and it's easy to understand what they do. So those were some of the first examples that I started writing. Maybe that kind of accidentally dominated the presentation of the extension.

[1:40] **Henry:** Right. It is a common thing for people to say they have thousands of tabs open, and there are extensions that help you with that.

[1:47] **Omar:** Right. So maybe it's unintentionally a good presentation. I think when you have a demo like this,  there's a lot of usefulness in being conservative in what the demo does, where it's something that people recognize as something useful.
...
[4:54] **Omar:** Clearly they're not thinking the same way you would if you were making a Python script or a shell script or something. They're thinking when you make an extension, it's an undertaking. It's very similar to trying to make an iPhone app or an Android app, where you have to make a project. You have to open it in this IDE and there's all this stuff around it. Even if you only want to do something very simple,

[5:15] **Henry:** Right. The whole boilerplate concept, I suppose. This is true even with making a web app now. I guess you could just make an HTML file, but..
...
[8:40] **Omar:** Now it is basically pure C and pure JavaScript. There's no package manager. There's no build step. The browser extension is one JavaScript file. And it's just plain ES6. And the C file system half, which is the half that talks to your operating system.. You do have to compile it, but it's just one C file. And it only depends on [FUSE](https://en.wikipedia.org/wiki/Filesystem_in_Userspace), which is the module that you need to make file systems for your computer.
...
[10:19] **Omar:** Right. That's an interesting part of this is that this is a project that I basically was working on for a few years, very much on and off, obviously. But it was basically in my head. And so is that part of why it's turned out the way it has? Where it has very few dependencies and it's just these two files, because there is no division of labor between different people. And it has this very clear kind of, I can drive it to have the aesthetic that I want.
...
[19:03] **Henry:** It would be nice if it was more of an instinct that you would turn to that first, rather than turn to the issues. I've seen people talk about how in open-source we need not just onboarding, but a way of deep diving into code. Maybe you mentioned people putting one in to put a trace into the GitHub repo or just a program that you run that goes through what things are happening.
...
[20:45] **Omar:** Yeah, I think that this is taking some of the ideas and trying to modernize them. Well, I mean, everything is a file. That's a phrase that goes back to the original Unix. Today, even if you take an operating systems class, I TA'd operating systems in college at Stanford.
...
[23:00] **Omar:** Yeah, there's a lot of different directions that it could go in there. There's a direction of, what stuff do people want to do on top? And I think there's already a lot of stuff you can do on top.

[23:10] **Omar:** I've seen a couple of things that other people have done. There's this one that I linked from the page where somebody can scrape out what song is playing on YouTube music. Now you just have access to that through the file system. And then there's also expanding the sort of interface with the browser so that you can get the source code of pages, so that you can manipulate stuff from your terminal.
...
[24:10] **Omar:** There are extensions that do just that specific thing, right? Where you can use vim to edit text areas. So you can write email in Gmail or you can use Emacs to edit text area. I've seen those and that's a whole extension to do this pull request that's five lines of code. So I think there is a real use for it. And somebody who had a blog post about TabFS mentioned that as one of the first things they wanted.
...
[26:14] **Omar:** This has been stewing in my head for a few years, so I have maybe an advantage over a lot of people. And the other thing that I wanted to say about response from the community is because of the way that the launch went, where it was immediately number one on hacker news for a full day and a half. It was really, really popular. And then also mentioned in a bunch of other places. It immediately went from basically this project where I was the only one on the project. It was a private project that I've never linked anybody to. To having thousands of stars and dozens of issue reports and pull requests or something. So it was this very accelerated timeline for open source, where immediately I was suddenly the maintainer of this project with a bunch of people sending me stuff.

[26:57] **Omar:** But I think that in some ways makes a lot of the choices a lot clearer than when you have this kind of boiling frog situation where your project slowly grows and then over time, after a few months or a year you find that you're maintaining this thing. Where now to me, it's immediately clear that I need to figure out, how am I going to spend my time with this? What kind of commitments do I want to make?
